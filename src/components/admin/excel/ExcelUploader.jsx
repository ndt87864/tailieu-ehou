...
import { processExcelFile } from '../../utils/excelUploader';
...
const ExcelUploader = () => {
  ...
  return (
    <div className={`p-6 ${isDarkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'} rounded-lg shadow`}>
      ...
      {selectedFile && (
        <div className="mb-4">
          <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            File đã chọn: <span className="font-semibold">{selectedFile.name}</span> ({Math.round(selectedFile.size / 1024)} KB)
          </p>
        </div>
      )}
      
      {error && (
        <div className={`p-4 mb-4 text-sm rounded-lg ${isDarkMode ? 'bg-red-900/30 text-red-300' : 'bg-red-100 text-red-800'}`}>
          <p>{error}</p>
        </div>
      )}
      ...
      {result && (
        <div className={`mt-6 p-4 rounded-lg ${isDarkMode ? 'bg-green-900/30 text-green-200' : 'bg-green-50 text-green-800'}`}>
