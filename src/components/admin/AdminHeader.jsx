...
const AdminHeader = ({ title, user, setIsSidebarOpen }) => {
  ...
  return (
    <header className={`px-4 py-3 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'} shadow-sm flex items-center justify-between`}>
      <div className="flex items-center">
        <button
          className={`p-2 rounded-md ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'} md:hidden`}
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="relative" ref={userMenuRef}>
          ...
          {isUserMenuOpen && (
            <div className={`absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white'}`}>
              <div className={`px-4 py-2 ${isDarkMode ? 'border-b border-gray-700' : 'border-b border-gray-200'}`}>
