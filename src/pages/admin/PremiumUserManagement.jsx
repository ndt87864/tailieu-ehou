...
const PremiumUserManagement = () => {
  ...
  const availableIcons = [
    ...
    { name: 'default', label: 'Mặc định ', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
      </svg>
    )},
    { name: 'crown', label: 'Vương miện ', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    { name: 'shield', label: 'Khiên bảo vệ', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
      </svg>
    )},
    { name: 'trophy', label: 'Cúp vàng', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    { name: 'lightning', label: 'Tia chớp', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  component: (
    <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <text x="12" y="15" textAnchor="middle" fontSize="8" fontWeight="bold" fill="currentColor" stroke="none">VIP</text>
    </svg>
  )
}
,
    { name: 'heart', label: 'Trái tim', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
      </svg>
    )},
    { name: 'rocket', label: 'Đồng vàng $', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <circle cx="12" cy="12" r="9" strokeWidth="2" />
    ...
    { name: 'gift', label: 'Quà tặng', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" />
      </svg>
    )},
    { name: 'magic', label: 'Phép thuật', component: (
      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const fetchPremiumTiers = async () => {
    try {
      setLoading(true);
      const tiersQuery = query(collection(db, 'premium_tiers'), orderBy('price', 'asc'));
  ...
  const handleToggleStatus = async (tier) => {
    try {
      ...
      setSuccess(`${tier.isActive ? 'Tạm dừng' : 'Kích hoạt'} gói "${tier.name}" thành công!`);
      fetchPremiumTiers();
      ...
      setError('Không thể thay đổi trạng thái gói người dùng. Vui lòng thử lại sau.');
          ...
          <main className={`p-4 md:p-6 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-slate-100 text-gray-900'}`}>
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
                <div>
                  <h1 className="text-2xl font-bold mb-1">Quản lý gói người dùng cao cấp</h1>
              ...
              {/* Search */}
              <div className="mb-6">
                <div className={`relative rounded-lg overflow-hidden ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm`}>
                  <input
                    type="text"
                ...
                <div className={`${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white'}`}>
                  <div className={`px-6 py-4 ${isDarkMode ? 'bg-gray-700/50 border-b border-gray-700' : 'bg-gray-50 border-b border-gray-200'}`}>
                    ...
                    <p className={`mt-1 text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                      {loading ? 'Đang tải...' : `Hiển thị ${filteredTiers.length} gói người dùng`}
                    </p>
                  </div>
                  ...
                  {loading ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className={`mt-2 text-sm font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                        {searchQuery ? 'Không tìm thấy gói người dùng nào phù hợp' : 'Chưa có gói người dùng cao cấp nào'}
                      </p>
                      <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                        {searchQuery ? 'Thử tìm kiếm với từ khóa khác' : 'Nhấn nút "Thêm gói người dùng" để tạo gói mới'}
                      </p>
                    </div>
                  ) : (
                      ...
                      <table className={`min-w-full divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                        <thead>
                          <tr className={isDarkMode ? 'bg-gray-700/30' : 'bg-gray-50'}>
                            <th scope="col" className={`sticky top-0 px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300 bg-gray-700/50' : 'text-gray-500 bg-gray-50'}`}>
                              Tên gói
                            </th>
                            <th scope="col" className={`sticky top-0 px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300 bg-gray-700/50' : 'text-gray-500 bg-gray-50'}`}>
                              Quyền hạn
                            ...
                            <th scope="col" className={`sticky top-0 px-6 py-3 text-right text-xs font-medium uppercase tracking-wider ${isDarkMode ? 'text-gray-300 bg-gray-700/50' : 'text-gray-500 bg-gray-50'}`}>
                              Thao tác
                            </th>
                          </tr>
                        </thead>
                        <tbody className={`divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
                          {filteredTiers.map((tier) => (
                            <tr key={tier.id} className={isDarkMode ? 'hover:bg-gray-700/30' : 'hover:bg-gray-50'}>
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    ...
                                    className={`inline-flex items-center p-1 rounded-md transition-colors ${
                                      tier.isActive
                                        ? isDarkMode ? 'text-orange-400 hover:bg-gray-700 hover:text-orange-300' : 'text-orange-600 hover:bg-gray-100 hover:text-orange-700'
                                        : isDarkMode ? 'text-green-400 hover:bg-gray-700 hover:text-green-300' : 'text-green-600 hover:bg-gray-100 hover:text-green-700'
                                    }`}
                                    title={tier.isActive ? 'Tạm dừng gói' : 'Kích hoạt gói'}
                                  >
                                    {tier.isActive ? (
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            >
              <div className={`px-6 py-4 border-b ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                <div className="flex items-center justify-between">
                  <h3 className={`text-lg font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                    {isEditing ? 'Chỉnh sửa gói người dùng' : 'Thêm gói người dùng mới'}
                  </h3>
                  <button
                    type="button"
              ...
              <div className={`px-6 py-5 ${isDarkMode ? 'bg-gray-700 text-gray-200' : 'bg-gray-50 text-gray-800'} max-h-[70vh] overflow-y-auto`}>
                <form onSubmit={isEditing ? handleEditTier : handleAddTier}>
                  ...
                  <div className="mb-5">
                    ...
                    <div className="relative">
                      ...
                      >
                        <div className="flex items-center">
                          <div className={`mr-2 ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`}>
                            {getIconComponent(tierForm.icon)}
                          </div>
                          <span>{availableIcons.find(i => i.name === tierForm.icon)?.label || 'Chọn biểu tượng'}</span>
                        </div>
          ...
          <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            >
              ...
              <div className={`px-6 py-5 ${isDarkMode ? 'bg-gray-700 text-gray-200' : 'bg-gray-50 text-gray-800'}`}>
                ...
                <p className={`text-center text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-4`}>
                  Bạn có chắc chắn muốn xóa gói người dùng này không? Hành động này không thể hoàn tác.
                </p>
